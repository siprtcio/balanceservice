...
import (
	...
	"github.com/siprtcio/balanceservice/router/api"
)
...
func InitRoutes() map[string]*Host {
	...
	hosts[Conf.Server.PrivateDomainApi] = &Host{api.PrivRouters()}
	hosts["localhost"] = &Host{api.PrivRouters()}
	hosts["tasks.tiniyo_balanceservice"] = &Host{api.PrivRouters()}
	return hosts
}
